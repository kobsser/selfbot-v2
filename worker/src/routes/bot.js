...
export async function handleBot(db, env, url, request) {
  ...
  if (path === '/api/bot/config' && request.method === 'GET') {
    ...
    return json({ settings, accounts });
  }

  if (path === '/api/bot/state' && request.method === 'GET') {
    return json({ states: await db.all('SELECT * FROM bot_state WHERE account_id=?',
  ...
  if (path === '/api/bot/state' && request.method === 'POST') {
    ...
    for (const s of states) {
      await db.run(`INSERT INTO bot_state (account_id,group_id,last_meow_time,last_fish_time,pending_timer_expiry,updated_at)
        VALUES (?,?,?,?,?,datetime('now'))
        ON CONFLICT(account_id,group_id) DO UPDATE SET
          last_meow_time=excluded.last_meow_time, last_fish_time=excluded.last_fish_time,
          pending_timer_expiry=excluded.pending_timer_expiry, updated_at=datetime('now')`,
  ...
  if (path === '/api/bot/jobs/start' && request.method === 'POST') {
    ...
    const res = await db.run('INSERT INTO job_runs (run_id,status) VALUES (?,"running")', body.run_id || null);
  ...
  if (path === '/api/bot/jobs/complete' && request.method === 'POST') {
    ...
    await db.run(`UPDATE job_runs SET ended_at=datetime('now'), status=?, accounts_processed=?, actions_executed=? WHERE id=?`,
